...
namespace UI
{
    public partial class FormSolicitadMantenimientoAnual : Form
    {

        MantenimientoBLL MantenimientoBLO = new MantenimientoBLL();
        MecanicoBLL MecanicoBLO = new MecanicoBLL();
        public FormSolicitadMantenimientoAnual()
        {
            InitializeComponent();
        }

        private void FormSolicitadMantenimientoAnual_Load(object sender, EventArgs e)
        {
        ...
        private void CargarComboMecanicos()
        {
            try
            {
                List<Mecanico> LMecanicos = MecanicoBLO.ObtenerMecanicos().Where(m => m.Activo && m.TiposDeMantenimiento.Any(tm => tm.IdTipoMantenimiento.Equals((int)EnumTipoMantenimiento.Anual))).ToList();
        ...
        private void CargarDgvMantPedientes()
        {
            try
            {
                List<Mantenimiento> LMantenimientos = MantenimientoBLO.ObtenerTodos().Where(m => m.TipoMantenimiento.IdTipoMantenimiento.Equals((int)EnumTipoMantenimiento.Anual) && m.EstadoMantenimiento.IdEstadoMantenimiento.Equals((int)EnumEstadoMantenimiento.Pendiente)).ToList();
        ...
        private void CargarDgvMantEnCurso()
        {
            try
            {
                List<Mantenimiento> LMantenimientosEnCurso = MantenimientoBLO.ObtenerTodos().Where(m => m.TipoMantenimiento.IdTipoMantenimiento.Equals((int)EnumTipoMantenimiento.Anual) &&
